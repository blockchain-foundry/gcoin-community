...
export GCOINCLI=${BUILDDIR}/qa/pull-tester/run-gcoin-cli
export GCOIND=${REAL_GCOIND}
...
if [ "x${ENABLE_GCOIND}${ENABLE_UTILS}${ENABLE_WALLET}" = "x111" ]; then
  ...
  echo "No rpc tests to run. Wallet, utils, and gcoind must all be enabled"
fi
