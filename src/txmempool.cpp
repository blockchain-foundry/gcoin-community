...
public:
    ...
    void Read(CAutoFile& filein, const CFeeRate& minRelayFee)
    {
        ...
        if (numEntries <= 0 || numEntries > 10000)
            throw std::runtime_error("Corrupt estimates file.  Must have between 1 and 10k entires.");
...
void CTxMemPool::PrioritiseTransaction(const uint256 hash, const std::string strHash, double dPriorityDelta, int64_t nFeeDelta)
{
    {
        LOCK(cs);
