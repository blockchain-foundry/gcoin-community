 ...
 *  list
 */
template<typename T, typename A> inline unsigned int GetSerializeSize(const std::list<T, A>& l, int nType, int nVersion);
template<typename Stream, typename T, typename A> inline void Serialize(Stream& os, const std::list<T, A>& l, int nType, int nVersion);
template<typename Stream, typename T, typename A> inline void Unserialize(Stream& is, std::list<T, A>& l, int nType, int nVersion);
...
inline unsigned int GetSerializeSize(const std::list<T, A>& l, int nType, int nVersion)
{
    ...
    for (typename std::vector<T, A>::const_iterator li = l.begin(); li != l.end(); ++li)
        nSize += GetSerializeSize((*li), nType, nVersion);
    return nSize;
}
...
void Serialize(Stream& os, const std::list<T, A>& l, int nType, int nVersion)
{
    ...
    for (typename std::list<T, A>::const_iterator li = l.begin(); li != l.end(); ++li)
        ::Serialize(os, (*li), nType, nVersion);
}
...
void Unserialize(Stream& is, std::list<T, A>& l, int nType, int nVersion)
{
    ...
    while (nMid < nSize) {
        ...
        if (nMid > nSize)
            nMid = nSize;
        l.resize(nMid);
        for (typename std::list<T, A>::iterator li = l.begin(); li != l.end(); ++li)
            ::Unserialize(is, (*li), nType, nVersion);
    }
}
