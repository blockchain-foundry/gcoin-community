...
public:
    ...
    bool Undo(const CTransaction &tx, CBlock &block)
    {
        ...
        if (!color_license::IsExist(tx.color)) {
            LogPrintf("%s() fail : no license for color %u", __func__, tx.color);
            return false;
        }
        color_license::AddNumOfCoins(tx.color, -tx.vout[0].nValue);
        return true;
...
public:
                    ...
                    const CBlock *pblock, bool fNCheckFork)
    {
        ...
        BOOST_FOREACH(const CTxIn txin, tx.vin) {
            CTransaction wtx;
            uint256 hashtmp = 0;
            if (!GetTransaction(txin.prevout.hash, wtx, hashtmp, pblock, true))
                return RejectInvalidTypeTx("fetch input failed", state, 50);
            if (wtx.color != tx.color) {
                string addr = GetDestination(
                        wtx.vout[txin.prevout.n].scriptPubKey);
                if (!(wtx.type == MINT && wtx.color == 0) || addr == "" ||
        ...
        if (color_license::IsExist(tx.color) && !fNCheckFork) {
    ...
    bool Undo(const CTransaction &tx, CBlock &block)
    {
        CTransaction wtx;
        uint256 hashtmp = 0;
        if (!GetTransaction(tx.vin[0].prevout.hash, wtx, hashtmp, &block, true))
            return "";
        string pre_owner = GetDestination(
                wtx.vout[tx.vin[0].prevout.n].scriptPubKey);
        ...
        if (wtx.color == 0) {
            color_license::Remove(tx.color);
        } else {
            // case license transfer
            string pre_owner = GetDestination(
                    wtx.vout[tx.vin[0].prevout.n].scriptPubKey);
            if (pre_owner == "")
                return error("%s() : can't fetch address of pre tx", __func__);
...
string GetTxOutputAddr(const CTransaction& tx, int index)
{
...
string GetTxInputAddr(const CTransaction& tx, const CBlock *pblock)
{
    ...
    if (cache.get()->tx_hash != tx.GetHash()) {
        ...
        uint256 hashtmp = 0;
        if (!GetTransaction(
                tx.vin[0].prevout.hash, wtx, hashtmp, pblock, true)) {
            cache.get()->address = "";
        } else {
...

    {
        ...
        if (tx.type == NORMAL) {
            BOOST_FOREACH(const CTxIn txin, tx.vin) {
                CTransaction wtx;
                uint256 hashtmp = 0;
                if (!GetTransaction(txin.prevout.hash, wtx, hashtmp, NULL, true)) {
...
/*
        ...
        //  Fee rate become fixed, so we dont need to check this
        if (fRejectInsaneFee && nFees > ::minRelayTxFee.GetFee(nSize) * 10000)
            return error("AcceptToMemoryPool: : insane fees %s, %d > %d",
            hash.ToString(),
            nFees, ::minRelayTxFee.GetFee(nSize) * 10000);

        if (!tx.IsCoinBase() && tx.type == NORMAL) {
            bool color_0 = false;
            for (unsigned int i = 0; i < tx.vin.size(); i++) {
                CTransaction wtx;
                uint256 hashtmp = 0;

                if (!GetTransaction(tx.vin[i].prevout.hash, wtx, hashtmp, NULL, true))
                    return error("%s: can't find input", __func__);

                if (wtx.color != 0 && wtx.color != tx.color) {
                    LogPrintf("Illegal color at vin[%d]  tx color:%d   vin color:%d  \n", i, tx.color, wtx.color);
                    return -1;
                }
                if (wtx.color == 0) {
                    color_0 = true;
            ...
            if (!color_0) {
                if (tx.out_itself) {
                    if (nFees != (int64_t) ((tx.GetValueOut() - tx.vout[tx.vout.size() - 1].nValue) * nFeeRate) && tx.color != 0)
                        return error("%s: Incorrect Fees : %d  expect %d",
                                     __func__, nFees, (int64_t) ((tx.GetValueOut() - tx.vout[tx.vout.size() - 1].nValue) * nFeeRate));
                } else if (nFees != (int64_t) (tx.GetValueOut() * nFeeRate) && tx.color != 0)
                    return error("%s: Incorrect Fees : %d  expect %d",
                                 __func__, nFees, (int64_t) (tx.GetValueOut() * nFeeRate));
            }
        }
*/
...
bool GetTransaction(const uint256 &hash, CTransaction &txOut, uint256 &hashBlock, const CBlock *pblock,  bool fAllowSlow)
{
    ...
    if (pindexSlow) {
        ...
        for (map<uint256, CBlockIndex*>::const_iterator it = mapBlockIndex.begin(); it != mapBlockIndex.end(); ++it) {
            CBlock block;
            if (ReadBlockFromDisk(block, it->second)) {
                BOOST_FOREACH(const CTransaction &tx, block.vtx) {
                    if (tx.GetHash() == hash) {
                        txOut = tx;
                        hashBlock = it->second->GetBlockHash();
                        return true;
                    }
                }
            }
...
#ifdef ENABLE_GCOIN
        // undo the data for VoteList & MemberList & LicenseList
...
int64_t nGetValueIn(const CTransaction& tx, map<uint256, CTransaction>& txpool)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++) {
        ...
        if (txpool.count(tx.vin[i].prevout.hash) != 0) {
            wtx = txpool[tx.vin[i].prevout.hash];
        } else if (!GetTransaction(tx.vin[i].prevout.hash, wtx, hashtmp, NULL, true)) {
        ...
        if (wtx.color == 0)
            color_0 = true;
        value += wtx.vout[tx.vin[i].prevout.n].nValue;
    }
    if (color_0)
        return 0;
    return value;
        ...
        if (tx.IsCoinBase()) {
            ...
            if (tx.out_itself) {
                if (Fees != (int64_t)((tx.GetValueOut() - tx.vout[tx.vout.size() - 1].nValue) * nFeeRate) && tx.color != 0) {
                    LogPrintf("Illegal Transaction Fee : %d  expect : %d", VIN, (int64_t) ((tx.GetValueOut() - tx.vout[tx.vout.size() - 1].nValue) * nFeeRate));
                    return false;
                }
            } else if (Fees != (int64_t) (tx.GetValueOut() * nFeeRate) && tx.color != 0) {
                LogPrintf("Illegal Transaction Fee : %d  expect : %d", VIN, (int64_t) (tx.GetValueOut() * nFeeRate));
                return false;
            }
...
*/
    ...
    BOOST_FOREACH(const CTransaction& tx, block.vtx)
    {
        if (!CheckTransaction(tx, state))
            return error("%s() : CheckTransaction failed", __func__);
#ifdef ENABLE_GCOIN
        if (!fMissPreBlock && block.vtx[0] != tx)
            if (!CheckTransactionType(tx, state, &block, fNCheckFork))
                return error("%s() : CheckTransactionType failed", __func__);
#endif // ENABLE_GCOIN
    }
...
// Reconstruct MemberList, VoteList and LicenseList.
