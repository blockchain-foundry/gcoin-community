...
static const double nFeeRate = 0;
...
#ifdef ENABLE_GCOIN
    int64_t GetFee(size_t size) const { return 0; } // unit returned is satoshis
#else
...
public:
    ...
    // 2014/9/10 add, use to note whether the tx output to itself
    const bool out_itself;
    const tx_type type;
    ...
    IMPLEMENT_SERIALIZE(
        ...
        /*if (fRead) {
            *const_cast<type_Color*>(&color) = 0;
            *const_cast<bool*>(&out_itself) = false;
            *const_cast<tx_type*>(&type) = NORMAL;
            UpdateHash();
            //CDataStream stream(SER_DISK, CLIENT_VERSION);
            //const CTransaction *ss = this;
            //stream << *ss;
            //std::string s = stream.str();
            //std::cerr << ToHex(s) << std::endl;
        }
        if (fWrite) {
            READWRITE(*const_cast<type_Color*>(&color));
            READWRITE(*const_cast<bool*>(&out_itself));
...
public:
    ...
    std::string ToString() const;
    int64_t GetBlockStartTime() const
    {
        return (int64_t)vtx[0].nLockTime;
    }
};
