...
/** A signature creator for blockheader. */
class BlockHeaderSignatureCreator : public BaseSignatureCreator {
    const CBlock* block;
    int nHashType;
    const BlockHeaderSignatureChecker checker;

public:
    BlockHeaderSignatureCreator(const CKeyStore* keystoreIn, const CBlock* blockIn, int nHashTypeIn=SIGHASH_ALL);
    const BaseSignatureChecker& Checker() const { return checker; }
    bool CreateSig(std::vector<unsigned char>& vchSig, const CKeyID& keyid, const CScript& scriptCode) const;
};
...
bool SignBlockHeader(const CKeyStore& keystore, CBlock& block, int nHashType=SIGHASH_ALL);
