...
BlockHeaderSignatureCreator::BlockHeaderSignatureCreator(const CKeyStore* keystoreIn, const CBlock* blockIn, int nHashTypeIn) : BaseSignatureCreator(keystoreIn), block(blockIn), nHashType(nHashTypeIn), checker(block) {}
bool BlockHeaderSignatureCreator::CreateSig(std::vector<unsigned char>& vchSig, const CKeyID& address, const CScript& scriptCode) const
{
    CKey key;
    if (!keystore->GetKey(address, key))
        return false;
    uint256 hash = SignatureHash(scriptCode, *block, nHashType);
    if (!key.Sign(hash, vchSig))
        return false;
    vchSig.push_back((unsigned char)nHashType);
    return true;
...
bool SignBlockHeader(const CKeyStore& keystore, CBlock& block, int nHashType)
{
    if (block.vtx.size() == 0 || block.vtx[0].vout.size() == 0)
        return false;

    BlockHeaderSignatureCreator creator(&keystore, &block, nHashType);

    return ProduceSignature(creator, block.vtx[0].vout[0].scriptPubKey, block.scriptSig);
}
